...
def parse_json(json_lock, json_latest):
    ...
    while True:
        try:
            ...
            json_sock.connect((environ.get("HOST_978"), 30979))
...
def parse_raw(raw_lock, raw_latest):
    ...
    while True:
        try:
            ...
            raw_sock.connect((environ.get("HOST_978"), 30978))
...
def main():
    ...
    print("Waiting for /run/stats978/stats.json to be created...")
    while True:
        try:
            with open("/run/stats978/stats.json", "r") as _:
                break
    ...
    print(
        "/run/stats978/stats.json found\nWaiting for /run/stats978/polar_range.influx to be created..."
    )

    while True:
        try:
            with open("/run/stats978/polar_range.influx", "r") as _:
                break
        except FileNotFoundError:
            pass
        time.sleep(1)

    print("/run/stats978/polar_range.influx found\nStarting stats.py")
